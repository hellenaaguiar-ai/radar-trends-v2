...
function parseTrends(raw) {
  ...
  for (const block of blocks) {
    ...
    const fit = block.match(/Fit:\s*(Alta|Media|Baixa|Alta|Média|Baixa)/i)?.[1]
    const angle = block.match(/Angulo:\s*([\s\S]{10,300}?)(?=\nFormato:|\n---|\n\*\*|$)/i)?.[1]?.replace(/[*_]/g, '').trim().replace(/\n+/g, ' ')
    const format = block.match(/Formato:\s*([^\n]{3,60})/i)?.[1]?.trim()
    const timing = block.match(/Timing:\s*([^\n]{3,40})/i)?.[1]?.trim()
    if (title && fit) items.push({ title, fit: fit.charAt(0).toUpperCase() + fit.slice(1), angle: angle || '', format: format || '', timing: timing || '' })
  }
  return items
}

function parseReverse(raw) {
  const field = (label) => raw.match(new RegExp(`${label}[^\\n]*\\n([\\s\\S]+?)(?=\\n[A-Z]{3,}[:\\s]|---IDEIAS|$)`, 'i'))?.[1]?.trim() || ''
  const idea = (fmt) => {
    const escaped = fmt.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const block = raw.match(new RegExp(`${escaped}:[\\s\\S]+?(?=\\n(?:VIDEO LONGO|CARROSSEL|REEL):|$)`, 'i'))?.[0] || ''
    return {
...
function FitBadge({ fit }) {
  const map = { Alta: [C.sage, C.cream], Media: [C.amberDim, C.cream], Média: [C.amberDim, C.cream], Baixa: [C.aged, C.espresso] }
...
function IdeiaCard({ format, idea, url }) {
  ...
  const map = {
    ...
    'VIDEO LONGO': { label: 'Video Longo', bg: C.rust, color: C.cream },
...
function TrendsTab() {
  ...
  async function buscarTrends() {
    ...
    try {
      ...
      setResults(parsed)
      setStatus(parsed.length ? `${parsed.length} temas encontrados - ${parsed.filter(r => r.fit === 'Alta').length} com fit alto` : 'Nenhum resultado estruturado. Tente novamente.')
    } catch (e) { clearInterval(timer.current); setStatus('Erro de conexao. Tente novamente.') }
    setLoadingRun(false)
  }
  ...
  async function analisarTema() {
    ...
    try {
      ...
      setStatus(parsed.length ? `Tema analisado - Fit: ${parsed[0]?.fit}` : 'Concluido.')
      setTopic('')
    } catch (e) { setStatus('Erro de conexao. Tente novamente.') }
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
          {['todos', 'Alta', 'Media', 'Baixa'].map(f => (
            <FilterBtn key={f} active={filter === f} onClick={() => setFilter(f)}>
              {f === 'todos' ? 'Todos' : f === 'Alta' ? 'Fit Alto' : f === 'Media' ? 'Fit Medio' : 'Fit Baixo'}
            </FilterBtn>
          ))}
        </div>
      )}
      ...
      {/* Tema especifico */}
      <Label>Ou analise um tema especifico</Label>
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        <Input value={topic} onChange={e => setTopic(e.target.value)} onKeyDown={e => e.key === 'Enter' && analisarTema()} placeholder="Ex: etica no mercado de influencia, livros em alta, IA substituindo trabalho criativo..." />
      ...
      {filtered.length === 0 && !loadingRun && (
        <Card style={{ textAlign: 'center', padding: '40px 24px' }}>
          <p style={{ color: C.amberDim, fontSize: 13, lineHeight: 1.8, fontFamily: F }}>
            ...
            o que esta em alta e cruzar com as suas linhas editoriais.
          </p>
        </Card>
      )}
      {filtered.length > 0 && (
...
function ReverseTab() {
  ...
  const STEPS = [
    ...
    'Gerando ideias com a sua voz e opiniao...',
  ]
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 24 }}>
        <Label>Cole a URL do video de referencia</Label>
      ...
      {!result && !loading && (
        <Card style={{ textAlign: 'center', padding: '40px 24px' }}>
          <p style={{ color: C.amberDim, fontSize: 13, lineHeight: 1.8, fontFamily: F }}>
            Cole o link de um video que performou fora da curva.<br />
            O agente disseca a estrutura e gera ideias de<br />
            <strong style={{ color: C.leather }}>Reel</strong>, <strong style={{ color: C.leather }}>Video Longo</strong> e <strong style={{ color: C.leather }}>Carrossel</strong> com a sua voz e opiniao.
          </p>
        </Card>
      )}
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: 40 }}>
            <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.25em', color: C.amberDim, textTransform: 'uppercase', fontFamily: F, marginBottom: 10 }}>
              Hellena Aguiar · Agente de Conteudo
            </div>
